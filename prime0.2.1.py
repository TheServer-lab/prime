...
- NEW: Custom error system with inheritance and properties

Version: 0.2.1
Changes: Added comprehensive custom error system with inheritance, properties, and assertions
...
6. Custom Error System - User-defined error types with inheritance and properties
...
------
  ...
  python3 prime.py --demo-errors               # Demonstrate custom error system
...
# FROZEN OPCODE TABLE - DO NOT MODIFY (version 0.2.1)
OPCODES = (
    # Stack operations
    "PUSH_CONST",
    "LOAD",
...
class Emitter:
    ...
    @staticmethod
    def load_pbc_from_bytes(data):
        """Load PBC from bytes (for embedded executables)."""
        import io
        import json
...
class CustomErrorType:
    """Represents a user-defined error type."""
    ...
    def __repr__(self):
        return f"<ErrorType {self.name} extends {self.parent}>"

class CustomErrorObject(ErrorObject):
    """Extended error object with custom properties."""
    
    def __init__(self, type_name, message=None, trace=None, **kwargs):
        super().__init__(type_name, message, trace)
        self.properties = kwargs
    
    def __repr__(self):
        props = ", ".join(f"{k}={v!r}" for k, v in self.properties.items())
        if props:
            return f"<Error {self.type}: {self.message} {{{props}}}>"
        return f"<Error {self.type}: {self.message}>"
...
class Parser:
    ...
    def statement(self):
        ...
        if p.type == "KW" and p.value == "error":
            self.error_declaration()
            return
            
        # Enhanced throw statement
        if p.type == "KW" and p.value == "throw":
            self.throw_enhanced()
            return
            
        # Assert statement
        if p.type == "KW" and p.value == "assert":
    ...
    def error_declaration(self):
        """Parse custom error type declaration: error Name [extends Parent] [with properties]"""
        p = self.expect("KW", "error")
        ...
        if self.accept("KW", "extends"):
            parent = self.expect("ID").value
        
        # Check for with clause (properties)
        if self.accept("KW", "with"):
            ...
            while True:
                ...
                self.em.emit("STORE", temp_var, loc=self.cur_line())
                properties[prop_name] = temp_var
        ...
        # Emit code to define the error type
        # Push properties
        prop_count = len(properties)
        for prop_name, temp_var in properties.items():
            self.em.emit("PUSH_CONST", prop_name, loc=p.line)
            self.em.emit("LOAD", temp_var, loc=p.line)
        
        # Push parent and name
        self.em.emit("PUSH_CONST", parent, loc=p.line)
        self.em.emit("PUSH_CONST", name, loc=p.line)
        
        # Call define_error (property count * 2 + 2 args)
        self.em.emit("CALL", ("define_error", prop_count * 2 + 2), loc=p.line)

    def throw_enhanced(self):
        """Parse enhanced throw statement with properties."""
        ...
        if self.peek().type == "ID":
            ...
            # Could be an expression that evaluates to an error object
            self.expression()
        ...
        if self.peek().type == "SYM" and self.peek().value == "(":
            ...
            if not (self.peek().type == "SYM" and self.peek().value == ")"):
                self.expression()
                message_var = self.em.new_temp()
                self.em.emit("STORE", message_var, loc=self.cur_line())
                properties["__message__"] = message_var
                ...
                if self.peek().type == "SYM" and self.peek().value == ",":
                    ...
                    while True:
                        if self.peek().type == "ID" and self._peek_is_sym("="):
                            # Named property
                            ...
                            # Positional argument - add to message
                            self.expression()
                            # Create formatted message
                            self.em.emit("CALL", ("str.format", 2), loc=self.cur_line())
                            message_var = self.em.new_temp()
                            self.em.emit("STORE", message_var, loc=self.cur_line())
                            properties["__message__"] = message_var
            ...
            # No properties, just type
            self.em.emit("PUSH_CONST", None, loc=p.line)  # Default message
            message_var = self.em.new_temp()
            self.em.emit("STORE", message_var, loc=self.cur_line())
            properties["__message__"] = message_var
        
        # Emit code to create and throw the error
        # Push error type
        self.em.emit("PUSH_CONST", error_type, loc=p.line)
        
        # Push message
        self.em.emit("LOAD", properties.get("__message__"), loc=p.line)
        
        # Push properties count
        prop_count = len(properties) - 1 if "__message__" in properties else 0
        
        # Push property key-value pairs
        for prop_name, temp_var in properties.items():
            if prop_name != "__message__":
                self.em.emit("PUSH_CONST", prop_name, loc=p.line)
                self.em.emit("LOAD", temp_var, loc=p.line)
        
        # Call create_error (args: type, message, property pairs...)
        self.em.emit("CALL", ("create_error", 2 + prop_count * 2), loc=p.line)
        
        # Throw it
        self.em.emit("THROW", None, loc=p.line)

    def assert_statement(self):
        """Parse assert statement: assert condition [, error_type [, message [, properties]]]"""
        p = self.expect("KW", "assert")
        
        # Parse condition
        self.expression()
        condition_var = self.em.new_temp()
        self.em.emit("STORE", condition_var, loc=p.line)
        
        # Parse optional error type and message
        error_type = "AssertionError"
        message = "Assertion failed"
        
        if self.accept("SYM", ","):
            # Has error type or message
            if self.peek().type == "STR" or (self.peek().type == "ID" and self.peek().value in ["true", "false", "null"]):
                # Message only (backward compatibility)
                self.expression()
                message_var = self.em.new_temp()
                self.em.emit("STORE", message_var, loc=p.line)
                
                self.em.emit("LOAD", condition_var, loc=p.line)
                self.em.emit("PUSH_CONST", error_type, loc=p.line)
                self.em.emit("LOAD", message_var, loc=p.line)
                self.em.emit("CALL", ("assert", 3), loc=p.line)
                return
            elif self.peek().type == "ID":
                # Error type
                error_type_tok = self.advance()
                error_type = error_type_tok.value
                ...
                if self.accept("SYM", ","):
                    ...
                    self.em.emit("PUSH_CONST", message, loc=p.line)
                    message_var = self.em.new_temp()
                    self.em.emit("STORE", message_var, loc=p.line)
        ...
        # Simple assert with default error
        self.em.emit("LOAD", condition_var, loc=p.line)
        self.em.emit("PUSH_CONST", error_type, loc=p.line)
        self.em.emit("PUSH_CONST", message, loc=p.line)
        self.em.emit("CALL", ("assert", 3), loc=p.line)
...
class PrimeVM:
    """PRIME Virtual Machine with custom error support."""
    ...
    def __init__(self, code, functions, debug, exports=None, cwd=None, sandbox=False):
        ...
        self.custom_error_types = {}
        self.cwd = cwd or os.getcwd()
        
        # Add custom error builtins
        self._add_custom_error_builtins()
    
    def define_error_type(self, name, parent="Error", properties=None):
        """Define a custom error type."""
        if name in self.error_parents:
            err = ErrorObject("NameError", f"Error type '{name}' already exists", trace=self._build_trace())
            raise err
        
        # Validate parent exists
        if parent not in self.error_parents:
            # Check custom error types
            if parent not in self.custom_error_types:
                err = ErrorObject("ValueError", f"Parent error type '{parent}' not found", trace=self._build_trace())
                raise err
        
        self.error_parents[name] = parent
        self.custom_error_types[name] = CustomErrorType(name, parent, properties or {})
        return None
    
    def create_custom_error(self, error_type, message=None, **kwargs):
        """Create a custom error object."""
        # Check if it's a built-in error type
        if error_type in self.error_parents and error_type not in self.custom_error_types:
            # Built-in error - use regular ErrorObject
            return ErrorObject(error_type, message)
        
        # Custom error - check if defined
        if error_type not in self.custom_error_types:
            # Auto-define it with parent "Error"
            self.define_error_type(error_type, "Error")
        
        # Create custom error object
        err_obj = CustomErrorObject(error_type, message, **kwargs)
        
        # Apply default properties from error type definition
        error_type_def = self.custom_error_types.get(error_type)
        if error_type_def:
            for prop_name, default_value in error_type_def.properties.items():
                if prop_name not in kwargs:
                    err_obj.properties[prop_name] = default_value
        
        return err_obj
    
    def get_error_hierarchy(self, error_type):
        """Get the complete inheritance hierarchy for an error type."""
        hierarchy = []
        current = error_type
        visited = set()
        ...
        while current is not None and current not in visited:
            ...
            current = self.error_parents.get(current)
        
        return hierarchy
    
    def is_error_type(self, obj, error_type):
        """Check if an object is an error of a specific type."""
        if not isinstance(obj, (ErrorObject, CustomErrorObject)):
            return False
        
        if obj.type == error_type:
            return True
        
        # Check inheritance
        hierarchy = self.get_error_hierarchy(obj.type)
        return error_type in hierarchy
    
    def error_matches(self, errobj: ErrorObject, handler_type: str):
        """Check if error matches handler type (with custom error inheritance)."""
        if isinstance(errobj, CustomErrorObject):
            # Check custom error inheritance
            hierarchy = self.get_error_hierarchy(errobj.type)
            if handler_type in hierarchy:
                return True
        
        # Original logic for built-in errors
        cur = errobj.type
        while True:
            if cur == handler_type:
                return True
    ...
    def _add_custom_error_builtins(self):
        """Add custom error-related builtins."""
        
        def bw_define_error(type_name, parent="Error", properties=None):
            return self.define_error_type(type_name, parent, properties)
        
        def bw_create_error(type_name, message=None, **kwargs):
            return self.create_custom_error(type_name, message, **kwargs)
        
        def bw_is_error(obj, error_type=None):
            if error_type is None:
                return isinstance(obj, (ErrorObject, CustomErrorObject))
            return self.is_error_type(obj, error_type)
        
        def bw_error_info(error_obj):
            if not isinstance(error_obj, (ErrorObject, CustomErrorObject)):
                err = ErrorObject("TypeError", "Expected error object", trace=self._build_trace())
                raise err
            ...
            info = {
                ...
                "hierarchy": self.get_error_hierarchy(error_obj.type)
            }
            
            if isinstance(error_obj, CustomErrorObject):
                info["properties"] = error_obj.properties
            
            return info
        
        def bw_error_parent(error_type):
            if error_type not in self.error_parents:
                err = ErrorObject("ValueError", f"Error type '{error_type}' not found", trace=self._build_trace())
                raise err
            return self.error_parents.get(error_type)
        
        def bw_error_types():
            all_types = set(self.error_parents.keys())
            all_types.update(self.custom_error_types.keys())
            return sorted(all_types)
        
        def bw_assert(condition, error_type="AssertionError", message=None, **kwargs):
            if not condition:
                if message is None:
                    message = "Assertion failed"
                err = self.create_custom_error(error_type, message, **kwargs)
                raise err
            return None
        
        self.builtins.update({
            "define_error": bw_define_error,
            "create_error": bw_create_error,
            "is_error": bw_is_error,
            "error_info": bw_error_info,
            "error_parent": bw_error_parent,
            "error_types": bw_error_types,
            "assert": bw_assert,
        })
    ...
    def _make_builtins(self, sandbox=False):
        ...
        built = {
            ...
            "AssertionError": make_error_ctor("AssertionError"),
            ...
            if op == "PUSH_CONST":
                ...
                if isinstance(obj, dict):
                    ...
                    if arg == "type":
                        ...
                        self._raise_error(err)
                elif isinstance(obj, CustomErrorObject):
                    if arg == "type":
                        self.stack.append(obj.type)
                    elif arg == "message":
                        self.stack.append(obj.message)
                        ...
                        self.stack.append(obj.trace)
                    elif arg in obj.properties:
                        self.stack.append(obj.properties[arg])
                    else:
                        err = ErrorObject("AttributeError", f"Error object has no attribute '{arg}'", trace=self._build_trace())
                        self._raise_error(err)
                else:
                ...
                if not isinstance(errobj, (ErrorObject, CustomErrorObject)):
...
class EmbeddedVM:
    def __init__(self, code, functions, debug, exports=None, filename="<embedded>"):
        self.code = code
        self.functions = functions
        self.debug = debug
        self.exports = exports or set()
        ...
        self.ip = 0
        self.stack = []
        self.frames = [{{"vars": {{}}, "consts": set(), "return_ip": None, "func_name": "<global>", "entry_ip": 0, "deferred": []}}]
        self.exception_stack = []
        self.builtins = self._make_builtins()
    
    def _make_builtins(self):
        # Minimal builtins for standalone execution
        return {{
            "say": lambda *args: print(*args),
            "read": lambda prompt=None: input(prompt or ""),
            "len": lambda x: len(x),
            "get": lambda coll, idx: coll[idx],
            "__mklist__": lambda *args: list(args),
            "__mkdict__": lambda *args: {{args[i]: args[i+1] for i in range(0, len(args), 2)}},
        }}
    ...
    def run(self):
        # Simplified VM execution for standalone mode
        while 0 <= self.ip < len(self.code):
            ...
            if op == "PUSH_CONST":
                ...
                if name in self.builtins:
                    ...
                    func_addr, params = self.functions[name]
                    param_bindings = {{p: args[i] if i < len(args) else None for i, p in enumerate(params)}}
                    self.frames.append({{
                        "vars": param_bindings,
                        "consts": set(),
                        "return_ip": self.ip,
    ...
    def _raise_error(self, errobj):
        print(f"Error: {{errobj}}")
        sys.exit(1)

# Helper function to load PBC from bytes
def load_pbc_from_bytes(data):
    """Load PBC data from bytes."""
    f = io.BytesIO(data)
    ...
    for _ in range(instr_count):
        opid = struct.unpack(">H", f.read(2))[0]
        # Map opid to opcode name
        opcode_names = [
            "PUSH_CONST", "LOAD", "STORE", "STORE_CONST", "PRINT",
            "ADD", "SUB", "MUL", "DIV", "MOD", "UNARY_NEG", "NOT",
            "AND", "OR", "CMP_EQ", "CMP_NE", "CMP_LT", "CMP_GT", "CMP_LE", "CMP_GE",
            "JMP", "JMP_IF_FALSE", "CALL", "RET", "GET_ATTR", "CALL_ATTR",
            "TRY", "THROW", "DEFER", "HALT"
        ]
        oplabel = opcode_names[opid] if opid < len(opcode_names) else f"OP_{{opid}}"
        arg_len = struct.unpack(">I", f.read(4))[0]
        arg_json = f.read(arg_len).decode("utf-8")
        arg = json.loads(arg_json)
        code.append((oplabel, arg))
...
# Decompress and load embedded bytecode
try:
    compressed = base64.b85decode("{encoded}")
    pbc_data = zlib.decompress(compressed)
    
    # Load the bytecode
    code, functions, debug, filename, exports, version = load_pbc_from_bytes(pbc_data)
    
    # Create and run VM
    vm = EmbeddedVM(code, functions, debug, exports, filename)
    vm.run()
    
except Exception as e:
    print(f"Error running PRIME executable: {{e}}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
def pretty(value):
    ...
    if isinstance(value, CustomErrorObject):
        props = ", ".join(f"{k}={v!r}" for k, v in value.properties.items())
        if props:
            return f"<{value.type}: {value.message} {{{props}}}>"
        return f"<{value.type}: {value.message}>"
...
def demonstrate_custom_errors():
    """Show examples of custom error usage in PRIME."""
    
    examples = {
        "basic_errors": '''
// Define custom error types
error ValidationError extends ValueError with {
    field = "unknown",
    value = null
}

error NetworkError extends IOError with {
    url = "",
    status = 0,
    timeout = false
}
...
func validate_age(age) {
    if age < 0 {
        throw ValidationError("Age cannot be negative", field="age", value=age)
    }
    if age > 150 {
        throw ValidationError("Age is unrealistic", field="age", value=age)
    }
    say "Age is valid:", age
}

// Error catching with inheritance
attempt {
    validate_age(-5)
    ...
    say "Generic value error:", e.message
}

// Assert with custom errors
assert 1 == 2, ValidationError, "Numbers should be equal", field="comparison", value="1==2"
''',

        "error_hierarchy": '''
// Complex error hierarchy
error BaseError extends Error
error DatabaseError extends BaseError with { query = "", database = "default" }
error ConnectionError extends DatabaseError with { host = "localhost", port = 5432 }
error TimeoutError extends ConnectionError with { timeout_seconds = 30 }

func query_database(query, host) {
    if host == "slow.server" {
        throw TimeoutError(
...
attempt {
    query_database("SELECT * FROM users", "slow.server")
} catch DatabaseError e {
    say "Database error occurred:"
    say "Query:", e.query
    say "Host:", e.host
    if e.type == "TimeoutError" {
        say "Timeout after", e.timeout_seconds, "seconds"
    }
}
''',
...
define_error("CustomError", "RuntimeError", { code = 0, severity = "medium" })

let err = create_error("CustomError", "Something went wrong", code=42, severity="high")
say "Error type:", err.type
say "Error code:", err.code
say "Error severity:", err.severity
say "Is CustomError?", is_error(err, "CustomError")
say "Is RuntimeError?", is_error(err, "RuntimeError")
say "Is ValueError?", is_error(err, "ValueError")

// Error info
let info = error_info(err)
say "Error hierarchy:", info.hierarchy
say "Error properties:", info.properties

// List all error types
say "Available error types:", error_types()
'''
    }
    ...
    for name, code in examples.items():
        ...
        try:
            ...
            vm = run_source_text(code, f"<example:{name}>")
...
def print_help():
    ...
    print("  python3 prime.py --demo-errors                    # Demonstrate custom error system")
...
def print_version():
    ...
    print("  - Custom error system with inheritance and properties")
    print("  - defer statement for cleanup")
    print("  - assert statement with custom errors")
...
def main():
  ...
  prime.py --demo-errors             # Demonstrate custom error system
        """
    )
    ...
    parser.add_argument("--demo-errors", action="store_true", help="Demonstrate custom error system")
